...
class ContextMenuMixin:
    ...
    def mostrar_menu_contextual(self, pos):
        ...
        ca_id = proxy_model.data(idx_score, Qt.UserRole + 1) or proxy_model.data(idx_score, Qt.UserRole)
        if not ca_id: return
        ...
        if obj_name == "tab_unified": # Candidatas
            menu.addAction(action_mover_fav) # Marcar Favorita
            menu.addAction(action_web)
            menu.addAction(action_mover_ofer) # Marcar Ofertada
            
            menu.addSeparator()
            self._add_red_action(menu, "Eliminar de la lista", FIF.DELETE, lambda: self._ocultar_de_candidatas(ca_id, nombre_ca))

        elif obj_name == "tab_seguimiento": # Favoritas
            # Opción: Mover a Ofertada (Nuevo)
            menu.addAction(action_mover_ofer)
            
            # Opción: Dejar de seguir
            act_unfav = QAction(FIF.UNPIN.icon(), "Eliminar de seguimiento", self)
            ...
            # Opción: Mover a Favoritas (Nuevo - regresa a seguimiento)
            menu.addAction(action_mover_fav)
            
            # Opción: Eliminar de ofertadas (regresa a nada)
            act_unofer = QAction(FIF.REMOVE_FROM.icon(), "Eliminar de ofertadas", self)
    ...
    def _add_red_action(self, menu, text, icon, slot):
        btn = QPushButton(f"  {text}"); btn.setIcon(icon.icon())
    ...
    def _abrir_web_por_id(self, ca_id): self.start_task(self.db_service.get_licitacion_by_id, self._open_url_callback, task_args=(ca_id,))
    def _open_url_callback(self, lic): 
        if lic and lic.codigo_ca: QDesktopServices.openUrl(QUrl(f"https://buscador.mercadopublico.cl/ficha?code={lic.codigo_ca}"))

    # IMPORTANTE: Usamos on_finished para refrescar la tabla inmediatamente
    ...
    def _ocultar_de_candidatas(self, cid, nombre):
        if QMessageBox.question(self, "Ocultar", f"¿Quitar de candidatas?\n{nombre}", QMessageBox.Yes|QMessageBox.No) == QMessageBox.Yes:
    ...
    def _agregar_nota_dialog(self, cid):
        text, ok = QInputDialog.getMultiLineText(self, "Nota Personal", "Escribe una nota:")
    ...
    def _borrar_nota(self, cid):
        if QMessageBox.question(self, "Borrar Nota", "¿Estás seguro?", QMessageBox.Yes|QMessageBox.No) == QMessageBox.Yes:
