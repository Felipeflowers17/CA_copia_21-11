...
from qfluentwidgets import (
    ...
    ProgressBar, InfoBar, InfoBarPosition, CheckBox, SpinBox, BodyLabel, LineEdit, MessageBox,
...
# --- COMPONENTE PERSONALIZADO: CHECKABLE COMBOBOX ---
...
class TableInterface(QWidget):
    ...
    def _show_filter_flyout(self):
        ...
        layout.addWidget(self.combo_states)
        ...
        def create_date_block(title, key_from, key_to, cal_from_attr, cal_to_attr):
            ...
            h_inputs.setSpacing(15)
            v_from = QVBoxLayout()
            v_from.setSpacing(2)
            v_from.addWidget(BodyLabel("Desde", self))
            cal_from = CalendarPicker()
            cal_from.setDateFormat(Qt.ISODate)
            setattr(self, cal_from_attr, cal_from) 
            v_from.addWidget(cal_from)
            v_to = QVBoxLayout()
            v_to.setSpacing(2)
            v_to.addWidget(BodyLabel("Hasta", self))
            cal_to = CalendarPicker()
            cal_to.setDateFormat(Qt.ISODate)
            setattr(self, cal_to_attr, cal_to) 
            v_to.addWidget(cal_to)
            h_inputs.addLayout(v_from)
            h_inputs.addLayout(v_to)
            v_layout.addLayout(h_inputs)
            return container
        ...
        btn_reset = ToolButton(FIF.DELETE, self)
        btn_reset.setToolTip("Limpiar Filtros")
...
class MainWindow(FluentWindow, ThreadingMixin, MainSlotsMixin, DataLoaderMixin, ContextMenuMixin, TableManagerMixin):
    ...
    def initNavigation(self):
        ...
        # --- NUEVO BOTÓN DE ACTUALIZACIÓN SELECTIVA ---
        self.navigationInterface.addItem(
            routeKey="update_web",
            icon=FIF.GLOBE, 
            text="Actualizar Info Web",
    ...
    def _show_update_flyout(self):
        """Muestra el Flyout para seleccionar qué pestañas actualizar desde la web."""
        view = QFrame()
        view.setObjectName("UpdateFlyout")
        ...
        layout.addWidget(StrongBodyLabel("Seleccionar qué actualizar:"))
        layout.addWidget(BodyLabel("Busca info nueva en Mercado Público.", self))
        
        self.chk_upd_candidatas = CheckBox("Candidatas")
        self.chk_upd_seguimiento = CheckBox("Seguimiento")
        self.chk_upd_ofertadas = CheckBox("Ofertadas")
        
        # Por defecto marcamos seguimiento y ofertadas
        self.chk_upd_seguimiento.setChecked(True)
        self.chk_upd_ofertadas.setChecked(True)
        
        layout.addWidget(self.chk_upd_candidatas)
        layout.addWidget(self.chk_upd_seguimiento)
        layout.addWidget(self.chk_upd_ofertadas)
        
        layout.addSpacing(10)
        
        btn_run = PrimaryPushButton("Actualizar Ahora")
        btn_run.clicked.connect(lambda: self._on_run_selective_update(view)) # Pasamos view para cerrarlo
        layout.addWidget(btn_run)
        
        target = self.navigationInterface.widget(routeKey="update_web")
        
        # CORRECCION: Usamos PULL_UP (o DROP_DOWN si ese fallara, pero PULL_UP es lo esperado)
    ...
    def _on_run_selective_update(self, flyout_view):
        ...
        if not scopes:
            InfoBar.warning("Atención", "Debes seleccionar al menos una opción.", parent=self)
            return
            
        # Cerrar Flyout (Ocultándolo)
        flyout_view.parent().close() 
        
        if self.is_task_running:
            InfoBar.warning("Ocupado", "Ya hay una tarea en ejecución.", parent=self)
            return
        ...
        self.start_task(
            ...
            on_result=lambda: logger.info("Actualización selectiva OK"),
            ...
            task_kwargs={"scopes": scopes}
        )

    def _connect_table_signals(self):
        ui = self.unifiedInterface
        ui.searchBar.textChanged.connect(lambda: self.filter_wrapper(self.table_unified, ui))
        ui.filtersChanged.connect(lambda: self.filter_wrapper(self.table_unified, ui))
        self.table_unified.customContextMenuRequested.connect(self.mostrar_menu_contextual)
        
        ui3 = self.seguimientoInterface
        ui3.searchBar.textChanged.connect(lambda: self.filter_wrapper(self.table_seguimiento, ui3))
        ui3.filtersChanged.connect(lambda: self.filter_wrapper(self.table_seguimiento, ui3))
        self.table_seguimiento.customContextMenuRequested.connect(self.mostrar_menu_contextual)
        
        ui4 = self.ofertadasInterface
        ui4.searchBar.textChanged.connect(lambda: self.filter_wrapper(self.table_ofertadas, ui4))
        ui4.filtersChanged.connect(lambda: self.filter_wrapper(self.table_ofertadas, ui4))
        self.table_ofertadas.customContextMenuRequested.connect(self.mostrar_menu_contextual)

    def filter_wrapper(self, table, ui_obj):
        self.filter_table_view(
            table, 
            ui_obj.searchBar.text(), 
            ui_obj.filter_state["2do_llamado"], 
            ui_obj.filter_state["monto"], 
            ui_obj.filter_state["show_zeros"], 
            ui_obj.filter_state["selected_states"],
            ui_obj.filter_state["pub_from"],
            ui_obj.filter_state["pub_to"],
            ui_obj.filter_state["close_from"],
            ui_obj.filter_state["close_to"]
        )

    def poblar_tab_unificada(self, data):
        super().poblar_tab_unificada(data)
        self.filter_wrapper(self.table_unified, self.unifiedInterface)
    ...
    def on_run_recalculate_thread(self, silent=False):
        ...
        if not silent:
            if MessageBox("Confirmar Recálculo", "Se recalcularán todos los puntajes.", self).exec(): self._start_recalc_task(silent)
        else: self._start_recalc_task(silent)

    def _start_recalc_task(self, silent):
        self.start_task(task=self.etl_service.run_recalculo_total_fase_1, on_finished=lambda: self.on_recalculate_finished_custom(silent))

    def on_recalculate_finished_custom(self, silent):
        self.set_ui_busy(False); self.on_load_data_thread(); InfoBar.success("Proceso Completado", "Puntajes actualizados.", parent=self)
    ...
    def on_auto_extract_yesterday(self):
        ...
        self.start_task(task=self.etl_service.run_etl_live_to_db, on_finished=self.on_auto_task_finished, task_args=({"mode":"to_db", "date_from":y, "date_to":y, "max_paginas":0},))

    @Slot()
    def iniciar_limpieza_silenciosa(self):
        self.start_task(task=self.etl_service.run_limpieza_automatica, on_result=lambda: None)
